...
const schema = Yup.object().shape({
  gas_consumption: Yup.number()
    // .positive('Valor deve ser maior que 0.')
    .required('Valor é obrigatório.'),
});

export default function GasFormModal({ handleClose, item, loadPlanning }) {
  const [initialData, setInitialData] = useState();
  const [isEdit, setEdit] = useState(false);
  const [planning, setPlanning] = useState([]);

  console.log('Form data gas', item); // eslint-disable-line
  ...
  async function handleSubmit(data) {
    ...
    toast.success('Valor de gás cadastrado com sucesso.');
